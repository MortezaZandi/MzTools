...
namespace WCFServer
{
    public class WCFServiceManager
    {
        ...
        public void RegisterService(string hostName, string serviceName, int portNumber, Type serviceClassTypem, Type servericeInterfaceType)
        {
            if (this.registeredServices.ContainsKey(hostName))
            {
                throw new InvalidOperationException($"Service '{serviceName}' already registered.");
            }

            var baseAddress = new Uri($"http://{hostName}:{portNumber}/{serviceName}/");

            var host = new WebServiceHost(CreateServiceInstance(serviceClassTypem), baseAddress);
