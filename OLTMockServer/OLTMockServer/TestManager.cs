...
namespace OLTMockServer
{
    public abstract class TestManager : IUIFactory
    {
        ...
        private void StartSending(CancellationToken cancellationToken, bool resume)
        {
            ...
            int totalRequiredActionsCount = 0;
            var countOfAutoGeneratedOrders = testProject.Orders.Count(o => o.IsAutoGenerated);

            var remainedOrdersToGenerate =
                testProject.TestOptions.GenerateOrdersAutomatically ?
                (testProject.TestOptions.MaxOrderCount - countOfAutoGeneratedOrders) : 0;

            var countOfNotProcessedOrders = testProject.Orders.Count(o => o.HasNotPrcessedActivity);

            totalRequiredActionsCount = (remainedOrdersToGenerate * 2) + countOfNotProcessedOrders;

            var performedAcounsCount = 0;
            ...
            while (continueExecution)
            {
                ...
                try
                {
                    if (testProject.Orders.Count > 0)
                    {
                        nextOrder = testProject.Orders[lastProcessingOrderIndex];

                        if (nextOrder.HasNotPrcessedActivity)
                        {
                            OrderProcessingFeedback?.Invoke(nextOrder, Definitions.OrderProcessingSteps.OrderSelectedForProcessing, totalRequiredActionsCount, performedAcounsCount);
                            ...
                            foreach (var activity in nextOrder.Activities)
                            {
                                if (!activity.IsDone && activity.TryCount < Definitions.Order_Max_Activity_Try_Count)
                                {
                                    ...
                                    OrderProcessingFeedback?.Invoke(nextOrder, Definitions.OrderProcessingSteps.PerformingOrderAcivity, totalRequiredActionsCount, performedAcounsCount, activity.ActivityType);
                                    ...
                                    if (Server.PerformOrderActivity(nextOrder, targetVendor, activity))
                                    {
                                        ...
                                        OrderProcessingFeedback?.Invoke(nextOrder, Definitions.OrderProcessingSteps.OrderAcivityPerformed, totalRequiredActionsCount, performedAcounsCount, activity.ActivityType);
                                    }
                                    else
                                    {
                                        nextOrder.AddLog($"Activity '{activity.ActivityType}' failed. {activity.LastTryExceptionMessage}", Definitions.LogTypes.Error);
                                        OrderProcessingFeedback?.Invoke(nextOrder, Definitions.OrderProcessingSteps.OrderAcivityNotPerformed, totalRequiredActionsCount, performedAcounsCount, activity.ActivityType);
                                    }
                    ...
                    {
                        countOfAutoGeneratedOrders = testProject.Orders.Count(o => o.IsAutoGenerated);
                ...
                catch (Exception ex)
                {
                    ...
                    OrderProcessingFeedback?.Invoke(
                        nextOrder,
                        Definitions.OrderProcessingSteps.OrderProcessingError, totalRequiredActionsCount, performedAcounsCount,
            ...
            {
                TestPlayStatuse = Definitions.TestPlayStatuses.Finished;
            }

            OrderProcessingFeedback?.Invoke(
                        null,
                        Definitions.OrderProcessingSteps.TestFinished, totalRequiredActionsCount, performedAcounsCount);
        }
